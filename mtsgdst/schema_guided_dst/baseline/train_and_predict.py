...
def _create_schema_embeddings(bert_config, schema_embedding_file,
  ...
  schema_json_path = os.path.join(FLAGS.dataset_dir, FLAGS.dataset_split,
                                  "schema.json")
...
def main(_):
  ...
  if FLAGS.run_mode == "train":
    ...
    input_json_files = [
        os.path.join(FLAGS.dataset_dir, FLAGS.dataset_split,
                     "dialogues_{:03d}.json".format(fid))
        for fid in dataset_config.file_ranges[FLAGS.dataset_split]
    ]
    schema_json_file = os.path.join(FLAGS.dataset_dir, FLAGS.dataset_split,
                                    "schema.json")
    ...
    for ckpt_num in ckpt_nums:
      ...
      dataset_mark = os.path.basename(FLAGS.dataset_dir)
